        ...
        contenedor.append(`
          <div class="mb-3">
            ...
            <select name="materia_${i}" class="form-select" required>
              <option value="">-- Selecciona NRC --</option>
              <?php
              // Obtener todos los NRCs disponibles desde la tabla Horario
              $sql = "SELECT DISTINCT h.NRC, m.nombre AS Materia, h.secc, h.dias, h.hora
                      FROM Horario h
              ...
              while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
                  $detalle = "NRC {$row['NRC']} - {$row['Materia']} - Secc {$row['secc']} - {$row['dias']} {$row['hora']}";
                  $options .= "<option value='{$row['NRC']}'>".htmlspecialchars($detalle)."</option>";
              }
              echo str_replace("\n", "", $options);
              ?>
            </select>
